...
void run_batched_parallel(const std::vector<ConfigType>& configs, TaskFunc task_func) {
    ...
    std::cout << "  [Parallelization: " << max_parallel << " concurrent tasks, " 
              << total_configs << " total configurations]" << std::endl;
...
template<typename T>
void save_ground_truth(const std::string& filename, const Output& output) {
...
std::unique_ptr<Output> load_ground_truth(const std::string& filename) {
    ...
    std::cout << "Ground truth loaded: " << filename 
...
void fill_with_ground_truth(Output& approx_output, const Output& ground_truth) {
    for (size_t i = 0; i < approx_output.queries.size() && i < ground_truth.queries.size(); i++) {
        ...
        size_t num_to_copy = std::min(approx_query.nearest_neighbors.size(), 
                                       gt_query.nearest_neighbors.size());
        
        for (size_t j = 0; j < num_to_copy; j++) {
            approx_query.nearest_neighbors[j].id_true = gt_query.nearest_neighbors[j].id_true;
            approx_query.nearest_neighbors[j].distance_true = gt_query.nearest_neighbors[j].distance_true;
        }
    }
}

// Print benchmark results in a formatted table
void print_benchmark_result(const std::string& algorithm, const std::string& params,
                           double avg_af, double recall, double qps, 
                           double t_approx, double speedup_vs_brute) {
    std::ostringstream oss;
    oss << std::left << std::setw(15) << algorithm
        << std::setw(30) << params
        << std::right
        << " AF=" << std::fixed << std::setprecision(4) << std::setw(7) << avg_af
        << " Recall=" << std::setw(6) << std::setprecision(3) << recall
        << " QPS=" << std::setw(8) << std::setprecision(1) << qps
        << " Time=" << std::setw(8) << std::setprecision(6) << t_approx << "s"
        << " Speedup=" << std::setw(6) << std::setprecision(1) << speedup_vs_brute << "x"
        << std::endl;
...
void write_csv_result(std::ofstream& csv_file, const std::string& dataset,
    ...
    oss << dataset << ","
        << algorithm << ","
        << params << ","
        << std::fixed << std::setprecision(6) << avg_af << ","
        << recall << ","
        << qps << ","
        << t_approx << ","
        << speedup_vs_brute << std::endl;
...
void run_lsh_benchmark(const Matrix<T>& input_images, const Matrix<T>& query_images,
                       const Output& ground_truth, ImageType img_type,
                       int N, double brute_time_per_query, std::ofstream& csv_file,
                       const std::string& dataset_name, bool enable_range_search = false, double R = 0.0) {
    ...
    auto task = [&](const std::tuple<int, int, double>& config) {
        ...
        calculate_metrics(*output, elapsed, 0.0);  // Pass approximate time, no brute force time in benchmark
        ...
        std::string params = "k=" + std::to_string(k) + " L=" + std::to_string(L) + 
                           " w=" + std::to_string(w);
        if (enable_range_search) {
            params += " R=" + std::to_string(R);
        }
        print_benchmark_result("LSH", params, avg_af, recall, qps, t_per_query, speedup);
        write_csv_result(csv_file, dataset_name, "LSH", params, avg_af, recall, qps, t_per_query, speedup);
    };
...
void run_hypercube_benchmark(const Matrix<T>& input_images, const Matrix<T>& query_images,
    ...
    auto task = [&](const std::tuple<int, int, int>& config) {
        ...
        write_csv_result(csv_file, dataset_name, "Hypercube", params, avg_af, recall, qps, t_per_query, speedup);
    };
...
void run_ivfflat_benchmark(const Matrix<T>& input_images, const Matrix<T>& query_images,
    ...
    std::cout << "  [Index builds needed: " << build_groups.size() << " (down from " << configs.size() << ")]" << std::endl;
    ...
    for (const auto& [k_clusters, nprobe_values] : build_groups) {
        ...
        std::cout << "  Built index: k_clusters=" << k_clusters 
        ...
        for (int nprobe : nprobe_values) {
            ...
            std::string params = "k_clusters=" + std::to_string(k_clusters) + " nprobe=" + std::to_string(nprobe);
            if (enable_range_search) {
                params += " R=" + std::to_string(R);
            }
            
            std::cout << "    nprobe=" << nprobe 
                      << " | Recall=" << std::setprecision(3) << recall
                      << " | Speedup=" << std::setprecision(1) << speedup << "x" << std::endl;
            
            write_csv_result(csv_file, dataset_name, "IVF-Flat", params, avg_af, recall, qps, t_per_query, speedup);
        }
    }
}
...
void run_ivfpq_benchmark(const Matrix<T>& input_images, const Matrix<T>& query_images,
    ...
    if (img_type == IMG_MNIST) {
        ...
        const std::vector<int> m_subvectors_values = {7, 8, 14, 16, 28, 49};    // 6 values (784/7=112, 784/8=98, etc.)
    ...
    // Then test all nprobe values on the same index (nprobe only affects querying, not building)
    std::cout << "  [Optimized: building index once per (k_clusters, m_subvectors, nbits) group]" << std::endl;
    ...
    for (const auto& [build_params, nprobe_values] : build_groups) {
        ...
        auto start_build = std::chrono::high_resolution_clock::now();
        Ivfpq<T, TWithNegatives, uint8_t> index(input_images, k_clusters, dims_per_subvector, nbits, 1);
        index.build();
        auto end_build = std::chrono::high_resolution_clock::now();
        double build_time = std::chrono::duration<double>(end_build - start_build).count();
        
        std::cout << "  Built index: k=" << k_clusters << " m=" << m_subvectors 
                  << " nbits=" << nbits << " (build time: " << std::fixed 
        ...
        for (int nprobe : nprobe_values) {
            ...
            std::string params = "k=" + std::to_string(k_clusters) + 
                               " nprobe=" + std::to_string(nprobe) + 
                               " m=" + std::to_string(m_subvectors) + 
            ...
            write_csv_result(csv_file, dataset_name, "IVF-PQ", params, avg_af, recall, qps, t_per_query, speedup);
        }
    }
}
...
template<typename T>
void run_dataset_benchmark(const std::string& dataset_name,
                          const std::string& input_path,
                          const std::string& query_path,
                          const std::string& gt_cache_path,
                          ImageType img_type,
                          int N = 10,
    ...
    if (!ground_truth) {
        std::cout << "No cached ground truth found. Computing (this will take a while)..." << std::endl;
        ...
        std::cout << "Brute force completed in " << std::fixed << std::setprecision(2) 
                  << elapsed << "s (" << std::setprecision(6) << brute_time_per_query 
        ...
        */
        ...
        brute_time_per_query = elapsed / query_images->get_rows();
        std::cout << "Brute force baseline: " << std::setprecision(6) 
                  << brute_time_per_query << "s per query" << std::endl;
    ...
    std::cout << "\n╔════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║         K-NEAREST NEIGHBOR BENCHMARKS (N=" << N << ")              ║" << std::endl;
    std::cout << "╚════════════════════════════════════════════════════════════╝" << std::endl;
    
    if (run_lsh) {
        run_lsh_benchmark(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, false, 0.0);
    }
    if (run_hypercube) {
        run_hypercube_benchmark(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, false, 0.0);
    }
    if (run_ivfflat) {
        run_ivfflat_benchmark(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, false, 0.0);
    }
    if (run_ivfpq) {
        if constexpr(std::is_same_v<T, uint8_t>) {
            run_ivfpq_benchmark<uint8_t, int8_t>(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, false, 0.0);
        }
        else {
            run_ivfpq_benchmark<T, T>(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, false, 0.0);
        }
    }
    ...
    if (enable_range_search) {
        std::cout << "\n╔════════════════════════════════════════════════════════════╗" << std::endl;
        std::cout << "║         RANGE SEARCH BENCHMARKS (R=" << R << ")                ║" << std::endl;
        std::cout << "╚════════════════════════════════════════════════════════════╝" << std::endl;
        
        if (run_lsh) {
            run_lsh_benchmark(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, true, R);
        }
        if (run_hypercube) {
            run_hypercube_benchmark(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, true, R);
        }
        if (run_ivfflat) {
            run_ivfflat_benchmark(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, true, R);
        }
        if (run_ivfpq) {
            if constexpr(std::is_same_v<T, uint8_t>) {
                run_ivfpq_benchmark<uint8_t, int8_t>(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, true, R);
            }
            else {
                run_ivfpq_benchmark<T, T>(*input_images, *query_images, *ground_truth, img_type, N, brute_time_per_query, csv_file, dataset_name, true, R);
            }
        }
    }
...
╚════════════════════════════════════════════════════════════╝
    ...
    if (run_mnist) {
        run_dataset_benchmark<uint8_t>(
            ...
            mnist_input,
            mnist_query,
            gt_dir + "/mnist_ground_truth_N10.bin",
            IMG_MNIST,
            10,
            enable_range_search,
            run_lsh,
    ...
    if (run_sift) {
        run_dataset_benchmark<float>(
            ...
            sift_input,
            sift_query,
            gt_dir + "/sift_ground_truth_N10.bin",
            IMG_SIFT,
            10,
            enable_range_search,
            run_lsh,

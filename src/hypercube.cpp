...
uint64_t HypercubeIndex<T>::get_hash_key(const T* data_point, size_t dim) const {
    ...
    for (int i = 0; i < kproj; ++i) {
        ...
        if (dot_product >= 0) {
            // using OR set the bit of the i-th bit to 1 if the point is on the positive side of
            // plane
            key |= (uint64_t(1) << i);
        }
    }
...
template <typename T> void HypercubeIndex<T>::build(const Matrix<T>& dataset) {
    std::cout << "Building Hypercube index for " << dataset.get_rows() << " points..." << std::endl;
    // build our map by taking all points in our dataset, hashing them and putting them in their
    // respective buckets
...
template <typename T>
std::vector<int> HypercubeIndex<T>::get_candidates(const T* query_point, size_t dim, int probes,
                                                   int M) const {
    //
...
template <typename T>
std::vector<ANearNeighbor> HypercubeIndex<T>::find_k_nearest(const T* query_point,
...
template <typename T>
std::vector<ImageId> HypercubeIndex<T>::find_in_range(const T* query_point,
                                                      const Matrix<T>& dataset, double R,
                                                      int probes, int M) const {
    std::vector<int> candidate_ids = get_candidates(query_point, dataset.get_cols(), probes, M);
...
std::unique_ptr<Output> hypercube_querying(const Matrix<T>& input_images,
    ...
    std::cout << "Processing " << query_images.get_rows() << " queries with Hypercube..."
              << std::endl;
    for (size_t i = 0; i < query_images.get_rows(); ++i) {
        ...
        std::vector<ANearNeighbor> neighbors = index.find_k_nearest(
            query_ptr, input_images, args.common.number_of_nearest, args.probes, args.M);
        ...
        if (args.common.search_for_range) {
            std::vector<ImageId> range_neighbors = index.find_in_range(
                query_ptr, input_images, args.common.radius, args.probes, args.M);
...
template std::unique_ptr<Output>
hypercube_querying<float>(const Matrix<float>&, const Matrix<float>&, HypercubeArguments&);
template std::unique_ptr<Output>
hypercube_querying<uint8_t>(const Matrix<uint8_t>&, const Matrix<uint8_t>&, HypercubeArguments&);
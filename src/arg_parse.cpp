...
static char usage_message[] = "Usage: %s <args> <algorithm>\n"
                              "\n"
...
static bool is_algorithm_argument(char* arg) {
    if (strcmp(arg, "-lsh") == 0 || strcmp(arg, "-hypercube") == 0 ||
        strcmp(arg, "-ivfflat") == 0 || strcmp(arg, "-ivfpq") == 0 ||
...
#define IF(X) else if (strcmp(argv[i], X) == 0)
    ...
    IF("-type") {
        char* given = argv[++i];
        if (strcmp(given, "sift") == 0)
            a->image_type = IMG_SIFT;
        else if (strcmp(given, "mnist") == 0)
            a->image_type = IMG_MNIST;
        else {
    ...
    IF("-range") {
        char* given = argv[++i];
        if (strcmp(given, "true") == 0)
            a->search_for_range = true;
        else if (strcmp(given, "false") == 0)
            a->search_for_range = false;
        else {

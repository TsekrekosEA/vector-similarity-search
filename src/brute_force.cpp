...
// If existing_output is provided, ONLY fills distance_true fields without modifying id or distance_approximate
template <typename T>
std::unique_ptr<Output> brute_force_querying(
    const Matrix<T>& input_images,
    const Matrix<T>& query_images,
    BruteforceArguments& args,
    std::unique_ptr<Output> existing_output
) {
    ...
    for (size_t i_query = 0; i_query < query_images.get_rows(); i_query++) {
        ...
        for (ImageId i_input = 0; i_input < input_images.get_rows(); i_input++) {
            ...
            double distance_squared = euclidean_distance_squared(query_pixels, input_pixels, query_images.get_cols());
        ...
        size_t N = (args.common.number_of_nearest > 0) ? static_cast<size_t>(args.common.number_of_nearest) : 1;
        ...
        if (args.common.search_for_range && !filling_existing) {
            ...
            std::partial_sort(euclidean_distances.begin(), 
                            euclidean_distances.begin() + sort_count,
        ...
        if (filling_existing) {
            ...
            for (size_t i = 0; i < N && i < euclidean_distances.size(); i++) {
                ...
                output->queries[i_query].nearest_neighbors[i].distance_true = std::sqrt(std::get<0>(euclidean_distances[i]));
                output->queries[i_query].nearest_neighbors[i].id_true = std::get<1>(euclidean_distances[i]);
            }
        } else {
        ...
        if (args.common.search_for_range && !filling_existing) {
            ...
            for (size_t i_near = 0; i_near < count_in_range; i_near++) {
                output->queries[i_query].r_near_neighbors.push_back(std::get<1>(euclidean_distances[i_near]));
            }
        }
...
template std::unique_ptr<Output> brute_force_querying<float>(
    const Matrix<float>&, const Matrix<float>&, BruteforceArguments&, std::unique_ptr<Output>);
template std::unique_ptr<Output> brute_force_querying<uint8_t>(
    const Matrix<uint8_t>&, const Matrix<uint8_t>&, BruteforceArguments&, std::unique_ptr<Output>);

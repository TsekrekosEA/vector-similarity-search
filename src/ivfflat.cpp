...
seed(seed),
pixels_per_image(dataset.get_cols()),
worth_clustering(dataset.get_rows() > static_cast<size_t>(k_clusters)),
dataset(dataset),
...
template <typename T>
void Ivfflat<T>::initialize_centroids_as_random_images(int seed) {
...
template <typename T>
void Ivfflat<T>::group_by_nearest_centroid() {
    ...
    for (size_t i_image = 0; i_image < dataset.get_rows(); i_image++) {
        ...
        for (size_t i_centroid = 0; i_centroid < centroids.get_rows(); i_centroid++) {
            double d_squared = eucl_d_sq_if_smaller_else_inf(
                centroids.get_row(i_centroid),
...
// 1. eucl_dist_squared_if_smaller(vec_1, vec_2, dim, other_distance) { ... if (sum > other_distance) return -1.0; }
// 2. auto arr = std::make_unique<int[]>(dataset.get_rows());

template <typename T>
void Ivfflat<T>::build() {

    // Can the dataset be empty? The class is used by IVFPQ for the inner centroids, which are chosen automatically. It does happen.
    ...
    for (int revisions_of_centroids = 0; revisions_of_centroids < max_centroid_revisions; revisions_of_centroids++) {
        ...
        for (size_t i_centroid = 0; i_centroid < centroids.get_rows(); i_centroid++) {
            ...
            if (images.empty()) continue;  // Skip empty clusters
            
            for (int i_pixel = 0; i_pixel < pixels_per_image; i_pixel++) {

                // TODO a template function with two types (ex. uint8_t, uint32_t) could refactor this code. It's harder than it looks however.
        ...
        if (should_print) {
            std::cout
            << label << " revision " << (revisions_of_centroids + 1) << "/" << max_centroid_revisions
            << " of the centroids, "
            << number_of_changed_centroid_pixels << "/"
            << changed_centroid_pixels_that_would_end_build
            << " centroid pixels modified                                                        \r";
            std::cout.flush();
        }


        if (number_of_changed_centroid_pixels <= changed_centroid_pixels_that_would_end_build) break;

        // Reconsidering the clusters.

        for (size_t i_centroid = 0; i_centroid < centroids.get_rows(); i_centroid++) {
            image_ids_per_cluster[i_centroid].clear();  // TODO Reallocations may cause a noticable performance hit
        }
...
template <typename T>
std::vector<std::tuple<double, int>> Ivfflat<T>::get_candidates(const T* pixels_of_query, int num_nearest_clusters, int num_results) const {

    // Find the nearest clusters.

    std::vector<std::tuple<double, int>> distances_and_centroid_ids;
    // TODO This memory doesn't need to be allocated and deallocated per query, it may be large and there will be many queries.
    distances_and_centroid_ids.reserve(centroids.get_rows());
    for (size_t centroid_id = 0; centroid_id < centroids.get_rows(); centroid_id++) {
        double d = euclidean_distance_squared(pixels_of_query, centroids.get_row(centroid_id), pixels_per_image);
    ...
    for (int i = 0; i < num_nearest_clusters && static_cast<size_t>(i) < distances_and_centroid_ids.size(); i++) {
    ...
    for (int i = 0; i < num_nearest_clusters && static_cast<size_t>(i) < distances_and_centroid_ids.size(); i++) {
        ...
        for (int image_id : image_ids_per_cluster[centroid_id]) {
            double d = euclidean_distance_squared(pixels_of_query, dataset.get_row(image_id), pixels_per_image);
    ...
    distances_and_image_ids.resize(
        num_results,
        std::make_tuple(std::numeric_limits<double>::infinity(), -1)  // TODO What should the default value be?
    );
...
double Ivfflat<T>::average_distance(const T * image, const std::vector<int> & image_ids) const {
    ...
    for (int id : image_ids) {
        sum_of_distances += euclidean_distance_squared(image, dataset.get_row(id), pixels_per_image);
    }
    return sum_of_distances / image_ids.size();
}

template <typename T>
double Ivfflat<T>::get_silhouette() const {
    ...
    for (size_t i_image = 0; i_image < dataset.get_rows(); i_image++) {
        ...
        if (should_print && (i_image + 1) % 100 == 0) {
            std::cout << label << " get_silhouette: image " << i_image + 1 << "/" << dataset.get_rows() << "\r";
            std::cout.flush();
        }
        ...
        for (size_t i_cluster = 0; i_cluster < centroids.get_rows(); i_cluster++) {
            double d = eucl_d_sq_if_smaller_else_inf(
                dataset.get_row(i_image),
                centroids.get_row(i_cluster),
                pixels_per_image,

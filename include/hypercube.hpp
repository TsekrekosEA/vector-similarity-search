...
public:
    ...
    std::vector<ANearNeighbor> find_k_nearest(const T* query_point, const Matrix<T>& dataset, int N, int probes, int M) const;
    // Add find_in_range for consistency
    std::vector<ImageId> find_in_range(const T* query_point, const Matrix<T>& dataset, double R, int probes, int M) const;

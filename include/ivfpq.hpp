...
class Ivfpq {
public:
    Ivfpq(const Matrix<T>& dataset, int k_clusters, int dimensions_of_sub_vectors, int nbits, int seed);
    Ivfpq(const Matrix<T>& dataset, int k_clusters, int dimensions_of_sub_vectors, int nbits, int seed, bool should_print);
    void build();
    std::vector<std::tuple<double, int>> get_candidates(const T* pixels_of_query, int num_nearest_centroids, int num_results) const;
    double get_silhouette() const { return coarse_clusters.get_silhouette(); }
    void print() const;
